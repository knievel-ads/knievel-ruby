...
module Knievel
  class ExplainResponse < ApiModelBase
    ...
    def self.openapi_types
      {
        ...
        :'explanation' => :'Hash<String, ExplainPlacement>'
      }
    end
    ...
    def initialize(attributes = {})
      if (!attributes.is_a?(Hash))
        fail ArgumentError, "The input argument (attributes) must be a hash in `Knievel::ExplainResponse` initialize method"
      end
      ...
      attributes = attributes.each_with_object({}) { |(k, v), h|
        if (!acceptable_attribute_map.key?(k.to_sym))
          fail ArgumentError, "`#{k}` is not a valid attribute in `Knievel::ExplainResponse`. Please check the name to make sure it's valid. List of attributes: " + acceptable_attribute_map.keys.inspect
        end
        h[k.to_sym] = v
      }
      ...
      if attributes.key?(:'explanation')
        if (value = attributes[:'explanation']).is_a?(Hash)
    ...
    def hash
      [snapshot_version, decisions, explanation].hash
    end
